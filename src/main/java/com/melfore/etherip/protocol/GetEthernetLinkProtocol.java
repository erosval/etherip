...
import com.melfore.etherip.data.InterfaceFlags;
...
public class GetEthernetLinkProtocol extends ProtocolAdapter
{
            ...
            final StringBuilder log) throws Exception
    {
        ...
        buf.get(raw);
        this.ethernetLink.setInterfaceSpeed(new Integer(raw[3] & 0xFF)
        ...
        raw = new byte[6];
        buf.get(raw);
        this.ethernetLink.setPhysicalAddress(
                String.format("%02X", raw[0]) + String.format(":%02X", raw[1])
                        + String.format(":%02X", raw[2])
                        + String.format(":%02X", raw[3])
                        + String.format(":%02X", raw[4])
                        + String.format(":%02X", raw[5]));
