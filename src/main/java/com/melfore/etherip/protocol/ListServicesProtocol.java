...
public class ListServicesProtocol extends ProtocolAdapter
{
    ...
    public static class Service
    {
        ...
        public Service(final short type, final short length,
                final short version, final short flags, final byte[] name)
        {
            this.type = type;
            this.length = length;
        ...
        public String toString()
        {
            ...
            msg.append("UINT length     : ").append(this.length).append("\n");
            msg.append(
                    String.format("UINT version    : 0x%04X\n", this.version));
            msg.append(String.format("UINT flags      : 0x%04X\n", this.flags));
            msg.append("UINT name[]     : ").append(this.name);
            return msg.toString();
        }
    };
            ...
            final StringBuilder log) throws Exception
    {
        ...
        this.services = new Service[count];
        for (int i = 0; i < count; ++i)
        {
            ...
            final Service service = new Service(type, length, version, flags,
                    name);
            if (log != null)
            {
