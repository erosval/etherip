...
public class MessageRouterPDUTest {
	...
	public void testGetAttrib() throws Exception {
		final MessageRouterProtocol pdu = new MessageRouterProtocol(CNService.Get_Attribute_Single,
				Identity().instance(0x24).attr(0x06), new ProtocolAdapter());
		assertEquals(pdu.getRequestSize(), 8);
		...
		assertEquals(Hexdump.toCompactHexdump(this.buf), "0000 - 0E 03 20 01 24 24 30 06 - .. .$$0.");

		// Fake response
		this.buf.clear();
		this.buf.put(new byte[] { (byte) 0x8E, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x49, (byte) 0x2C,
				(byte) 0x41, (byte) 0x00 });
		this.buf.flip();
	...
	public void testReadData() throws Exception {
		final MessageRouterProtocol pdu = new MessageRouterProtocol(CNService.CIP_ReadData, Symbol("kay_ai"),
				new ProtocolAdapter() {
					...
					@Override
					public void encode(final ByteBuffer buf, final StringBuilder log) { // Number of elements to read
						buf.putShort((short) 1);
						log.append("UINT elements : 1\n");
					}
				});
		assertEquals(pdu.getRequestSize(), 12);
		...
		assertEquals(Hexdump.toCompactHexdump(this.buf), "0000 - 4C 04 91 06 6B 61 79 5F 61 69 01 00 - L...kay_ai..");

		// Fake response
		this.buf.clear();
		this.buf.put(new byte[] { (byte) 0xCC, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0xCA, (byte) 0x00,
